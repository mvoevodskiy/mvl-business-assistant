...
class mvlbaCashController extends BotController {
    ...
    add_act = async (ctx) => {
        ...
        cash.amount = this.MT.numberFromHuman(cash.amount);
        ...
        switch (cash.direction) {
            ...
            case 'out':
            case 'accountable':
                cash.bankAccountId = cash.bankAccountIdOut || cash.bankAccountId || 0;
        ...
        Promise.all([
            (() => this.create(cash))(),
            (() => cash.direction === 'transfer' ? this.create(cash2) : true)()
        ])
            .then(async created => {
    ...
    async create (cash) {
        return this.Model.create(cash)
            .then(async created => {
                if (created !== null) {
                    await this.changeBankAccount(created.bankAccountId, created.amount);
                    return created;
                } else {
                    return null;
                }
